...
- 单文件和目录批处理都会在完整成功后自动删除原始 `.zip` 和配套 `.json`
- 目录批处理中的普通发布失败会把失败次数写入同名 JSON 的 `workflow.publish_failure` 字段；默认会保留前 **3** 次失败供排查和重跑，第 **4** 次普通发布失败会自动删除对应 `.zip` 和 `.json`
- `Invalid gallery` / 画廊已被替换（Replaced）仍然是立即清理路径，不占用普通发布失败次数额度
- 目录批处理的最终退出码现在只反映“是否仍存在未解决失败”；如果本轮失败项都已终局清理，则批处理会返回成功状态
...
当目录批处理中出现普通发布失败时，脚本会在现有 sidecar JSON 中自动追加（或更新）如下字段：
...
```json
{
  "workflow": {
    "publish_failure": {
      "count": 2,
      "latest_error": "上传到 e-hentai 失败",
      "latest_timestamp": "2026-04-23T10:30:00+08:00"
    }
  }
}
```

- `count`：当前 ZIP 的普通发布失败累计次数
- `latest_error`：最近一次普通发布失败摘要（会尽量保留真实错误原因，而不是固定文案）
- `latest_timestamp`：最近一次普通发布失败时间
- 这些字段由工作流自动维护，不需要手动编辑
...
### Q: 上传失败提示 "Invalid gallery" 怎么办？

这通常意味着画廊已被替换（Replaced）。e-hentai 上传者经常会重新上传相同或更好的版本，此时：
- 旧画廊会被标记为 **"Replaced"**
- 无法向旧画廊上传种子
- 脚本会自动检测这种情况并删除源文件，让下一轮重新生成

**行为**：
- 检测到画廊被替换 → 删除 `.zip` 和 `.json` 文件
- 输出新画廊 URL（如有）供参考
- 批处理模式下继续处理下一个文件
...
⚠️ 画廊已被替换（Replaced）
📌 找到更新版本：https://e-hentai.org/g/3837915/8b2a5a98e8/

⚠️ 画廊已被替换，将清理源文件让下一轮重新生成
💡 新画廊 URL: https://e-hentai.org/g/3837915/8b2a5a98e8/
🗑️ 删除：downloads/xxx.zip
🗑️ 删除：downloads/xxx.json
```

下一轮系统会自动重新生成新画廊的文件。

### Q: 普通上传失败会不会一直重试，导致坏文件永远留在目录里？

- 不会无限遗留。当前脚本会把**目录批处理中的普通发布失败次数**记录到同名 JSON 的 `workflow.publish_failure` 中
- 默认会保留前 **3** 次普通失败，方便人工排查、网络恢复后重跑、或等待临时问题自行恢复
- 当同一个 ZIP 在目录批处理中出现第 **4** 次普通发布失败时，脚本会在当次流程内自动删除对应 `.zip` 和 `.json`
- 这条“超阈值清理”路径属于终局清理，会在批处理汇总里单独展示，不再把该文件留给后续轮次反复重试
- 单文件模式不会累计这个普通发布失败预算，也不会因为累计次数而自动删除源文件；它仍然适合手动排查单个 ZIP

### Q: 目录批处理的退出码现在表示什么？

- **退出码 0**：本轮处理后，不存在仍需后续重试的未解决失败；也就是说，结果可能是“全部成功”，也可能是“成功 + 已终局清理”
- **退出码非 0**：本轮仍有未解决失败项留在目录中，后续轮次还会继续尝试这些文件
- 已被终局清理的条目（例如 replaced-gallery 或超过普通失败阈值后被删除）不会单独导致批处理保持非零退出
